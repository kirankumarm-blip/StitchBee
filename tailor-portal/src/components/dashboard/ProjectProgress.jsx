...
export default function ProjectProgress({ progressItems, weeklyChart }) {
  return (
    <div className="sb-dashboard-card" style={{ height: '100%' }}>
      ...
      <div style={{ marginTop: 'auto', paddingTop: '10px', borderTop: '1px solid var(--sb-border)' }}>
        ...
        <div style={{ width: '100%', height: '45px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={weeklyChart} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              <XAxis dataKey="day" stroke="var(--sb-text-secondary)" fontSize={10} tickLine={false} axisLine={false} />
              <Tooltip cursor={{ fill: 'rgba(0,0,0,0.04)' }} />
              <Bar dataKey="completed" fill="#F72585" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
